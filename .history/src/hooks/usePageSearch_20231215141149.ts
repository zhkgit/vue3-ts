...
export function usePageSearch(): [
    ...
    const handleAddData = () => {
        ...
        // modalRef.value.useFormInit(modalConfig.formItems ?? [])
        modalRef.value.formData = ref(modalRef.value.useFormInit(modalConfig.formItems ?? []))
    }
    const handleEditData = (value: any) => {
        console.log(value)
        modalRef.value.formData = value
        modalRef.value.dialogVisible = true
    }
    // 返回包含contentRef、handleResetClick和handleQueryClick的数组
    return [contentRef, handleResetClick, handleQueryClick, handleAddData, modalRef, handleEditData]
}
